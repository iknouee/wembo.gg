...
export default function DocsPage() {
  ...
  return (
    <div className="relative min-h-screen bg-[#050505] py-24 lg:py-32">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[400px] bg-[#FFD600]/[0.02] rounded-full blur-[100px]" />

      <div className="relative container mx-auto px-4 lg:px-8">
        <div className="max-w-6xl mx-auto">
          {/* Header */}
          <div className={`mb-12 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <span className="inline-flex items-center gap-2 text-[11px] font-semibold text-[#FFD600] uppercase tracking-wider mb-5 bg-[#FFD600]/[0.04] border border-[#FFD600]/20 rounded-full px-3 py-1.5">
              <BookOpen className="h-3.5 w-3.5 text-[#FFD600]" /> Documentation
            </span>
            <h1 className="text-[clamp(2rem,4vw,2.75rem)] font-bold text-white tracking-tight mb-4">
              Wembo Documentation
            </h1>
            <p className="text-lg text-[#9A9CA3]/60 max-w-2xl leading-relaxed">
          ...
          <div className={`mb-14 transition-all duration-700 delay-100 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            <div className="relative max-w-xl group">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-[#9A9CA3]/30 group-focus-within:text-[#FFD600]/50 transition-colors" />
              <input
                type="text"
                placeholder="Search documentation..."
                className="w-full h-12 rounded-xl border border-white/[0.03] bg-[#090A0C] pl-11 pr-4 text-sm text-[#F7F7F8] focus:outline-none focus:border-[#FFD600]/20 transition-all placeholder:text-[#9A9CA3]/30"
                readOnly
              />
            </div>
          </div>
            ...
            {docsSections.map((section, i) => (
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  <div className="rounded-lg bg-[#141519] p-2 group-hover:bg-[#1a1b20] transition-colors">
                    <section.icon className="h-4 w-4 text-[#9A9CA3] group-hover:text-[#FFD600] transition-colors" />
                  </div>
                  <h3 className="font-semibold text-[#F7F7F8]">{section.title}</h3>
                </div>
                <p className="text-sm text-[#9A9CA3]/50 mb-4">{section.description}</p>
                <ul className="space-y-2">
                  {section.articles.map((article) => (
                    <li key={article}>
                      <Link href="#" className="text-sm text-[#9A9CA3]/50 hover:text-white/70 transition-colors flex items-center gap-2 group/link">
                        <div className="h-1 w-1 rounded-full bg-[#9A9CA3]/20 group-hover/link:bg-[#FFD600] transition-colors" />
                        {article}
                      </Link>
                    </li>
                  ))}
          ...
          {/* Help CTA */}
          <div className={`mt-14 rounded-2xl bg-[#090A0C] p-8 lg:p-10 text-center transition-all duration-700 delay-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            <h3 className="text-xl font-bold mb-2 text-white">Need help?</h3>
            <p className="text-[#9A9CA3]/50 mb-5">Can&apos;t find what you&apos;re looking for? Join our Discord for support.</p>
            <Link href="https://discord.gg/Hgvs5WnZaK">
              <button className="inline-flex items-center gap-2.5 rounded-xl bg-[#5865F2] px-6 py-3 text-sm font-medium text-white hover:bg-[#4752C4] hover:-translate-y-0.5 transition-all duration-300">
