    ...
    <div className="p-6 lg:p-8 space-y-8 animate-in fade-in duration-300">
      ...
      <div className="space-y-6">
        <Section title="Detection">
          <Row label="Join Threshold" desc="Joins in window to trigger detection"><NI value={config.join_threshold} onChange={v => setConfig({ ...config, join_threshold: v })} min={3} max={50} /></Row>
          <Row label="Time Window" desc="Seconds to count joins"><div className="flex items-center gap-2"><NI value={config.time_window_seconds} onChange={v => setConfig({ ...config, time_window_seconds: v })} min={5} max={60} /><span className="text-[11px] text-white/15">sec</span></div></Row>
          <Row label="Min Account Age" desc="Flag accounts newer than" last><div className="flex items-center gap-2"><NI value={config.min_account_age_hours} onChange={v => setConfig({ ...config, min_account_age_hours: v })} min={0} max={720} /><span className="text-[11px] text-white/15">hrs</span></div></Row>
        </Section>
        <Section title="Response">
          <Row label="Action" desc="What to do with raiders"><AS value={config.action} onChange={v => setConfig({ ...config, action: v })} options={[{ value: 'kick', label: 'Kick' }, { value: 'ban', label: 'Ban' }, { value: 'lockdown', label: 'Lockdown' }]} /></Row>
          <Row label="Notify" desc="Alert in log channel" last><TG value={config.notify_channel} onChange={v => setConfig({ ...config, notify_channel: v })} /></Row>
        </Section>
      </div>
